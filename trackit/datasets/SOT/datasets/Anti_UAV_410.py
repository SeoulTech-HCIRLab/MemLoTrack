...
# # trackit/datasets/SOT/datasets/Anti_UAV_410.py
# import os
# import json
...
#         super().__init__('Anti_UAV_410', root_path, data_split, data_split, version=1)
...
#                 continue
#             total += sum(
#                 1 for seq in os.listdir(split_dir)
#                 if os.path.isdir(os.path.join(split_dir, seq))
#             )
...
#                 ann_path = os.path.join(seq_path, 'IR_label.json')
...
#                 valid_items = []
#                 for idx, (rect, ex_flag) in enumerate(zip(gt_rects, exist_list), start=1):
...
#                     seq_ctor.set_name(f"Anti_UAV_410_{seq_name}")
...
class Anti_UAV_410_Seed(BaseSeed):
    def __init__(self, root_path: str = None, data_split=('train','val','test')):
        if root_path is None:
            root_path = self.get_path_from_config('Anti_UAV_410_PATH')
        # ★ 캐시 무효화: 반드시 버전 올리세요
        super().__init__('Anti_UAV_410', root_path, data_split, data_split, version=3)

    def _count_images(self, seq_path: str) -> int:
        # 000001.jpg ... 연속 이미지 개수
        n = 0
        while os.path.isfile(os.path.join(seq_path, f'{n+1:06d}.jpg')):
            n += 1
        return n
    ...
    def construct(self, constructor: SingleObjectTrackingDatasetConstructor):
        ...
        for split in self.data_split:
            ...
            if os.path.isdir(split_dir):
                total += sum(1 for s in os.listdir(split_dir) if os.path.isdir(os.path.join(split_dir, s)))
        constructor.set_total_number_of_sequences(total)
                ...
                with constructor.new_sequence(category_id=0) as seq_ctor:
                    ...
                    for idx in range(1, T + 1):
                        ...
                        with seq_ctor.new_frame() as f_ctor:
                            ...
                            # validity=True → exist 프레임, False → non-exist 프레임(더미)
