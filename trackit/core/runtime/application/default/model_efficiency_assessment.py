...
def _run_model_number_of_parameters_assessment(model_manager: ModelManager, device: torch.device):
    ...
    # print('[ParamFlags] begin parameter requires_grad flags:')
    # for name, p in train_model.model.named_parameters():
    #     print(f'  {name}: requires_grad={p.requires_grad}')
    # print('[ParamFlags] end parameter requires_grad flags.')
    ...
    if trainable_num_parameters is not None:
...
def run_model_efficiency_assessment(model_manager: ModelManager, device: torch.device,
    ...
    print("[Debug] Entering run_model_efficiency_assessment, about to do latency/flops check")
    ...
    if model_manager.sample_input_data_generator is not None:
        ...
        try:
            ...
            print('Model FLOPs assessment is not supported: torch.jit.trace failed.')
            import traceback; traceback.print_exc()
        except Exception as e:
            print('Model FLOPs assessment threw an unexpected exception:')
            import traceback; traceback.print_exc()
            ...
            # ── 디버그: 전체 예외 스택 트레이스 출력 ──
            import traceback
            print('[FLOPs Debug] exception during flop count assessment:')
            traceback.print_exc()
            print(f'[FLOPs Debug] exception message: {e}')
