...
def bbox_clip_to_image_boundary_(bbox: np.ndarray, image_size: np.ndarray) -> np.ndarray:
    """
    Clips the bounding box coordinates to the image boundaries in-place.
    """

    bbox[..., 0] = np.clip(bbox[..., 0], 0, image_size[..., 0])  # x_min
    bbox[..., 1] = np.clip(bbox[..., 1], 0, image_size[..., 1])  # y_min
    bbox[..., 2] = np.clip(bbox[..., 2], 0, image_size[..., 0])  # x_max
    bbox[..., 3] = np.clip(bbox[..., 3], 0, image_size[..., 1])  # y_max
