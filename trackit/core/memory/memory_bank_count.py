...
class MemoryBank:
    ...
    def __init__(
        ...
        # ---- Stats ----
        self.write_count: int = 0           # 실제 저장(append 또는 replace) 횟수
        self.replace_count: int = 0         # 동일 frame_idx 교체 횟수
        self.drop_lowconf_count: int = 0    # conf < conf_thresh 로 drop된 횟수
    ...
    def add(
        ...
        *,
        ...
        Returns:
            bool: 실제로 저장(append 또는 replace)되었으면 True, 아니면 False
        """
        if conf < self.conf_thresh:
            # below threshold → not stored
            self.drop_lowconf_count += 1
            return False
