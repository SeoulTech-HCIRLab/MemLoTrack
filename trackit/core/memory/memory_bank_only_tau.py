...
Gating 정책 (Confidence-only)
- MemoryBank 자체는 confidence 임계치(conf_thresh)만 확인합니다.
- 동일 frame_idx가 이미 있으면 더 높은 conf일 때만 교체합니다(아니면 유지).
- 상위 파이프라인에서 conf ≥ conf_thresh 인 경우에만 add()를 호출하도록 설계되었습니다.
"""
...
class MemoryItem:
    ...
    Args:
        feat: (1, N, C) shaped tokens (e.g., pre-encoder or post-embed tokens)
        ...
        crop_params: (2,2) numpy array
            ...
            SiamFC 스타일 크롭에서 out = S * x + T 의 per-axis 버전.
...
class MemoryBank:
    """
    단순/정렬형 메모리 은행 (Confidence-only 게이팅).
    ...
    def add(
        ...
        *,
        ...
        동작 (confidence-only):
            ...
            - 동일 frame_idx 존재 시 더 높은 conf일 때만 교체
            - feat shape: (1, N, C)
        """
