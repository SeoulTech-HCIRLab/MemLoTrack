...
# 추가인자, force_adjacent 를 추가하여 True 일 경우 z_index + 1 만 반환하도록 수정 
def _get_search_frame_candidates(z_index: int, 
                                 length: int, 
                                 frame_range: Optional[int], 
    ...
    if sampling_method == SiamesePairSamplingMethod.causal:
        ...
        if force_adjacent:
            # 강제로 바로 다음 프레임만 후보로 사용
            return np.array([x_frame_begin]), False
...
def _do_siamfc_pair_sampling(length: int, frame_range: Optional[int], mask: np.ndarray = None,
    ...
    for _ in range(frame_range_auto_extend_max_retry_count + 1):
        x_candidate_indices, frame_range_extendable = _get_search_frame_candidates(z_index, length, frame_range, mask, sampling_method, force_adjacent)
