...
def positive_sample_assignment(bbox: np.ndarray, response_map_size: np.ndarray, search_region_size: np.ndarray):
    ...
    if len(positive_sample_indices) == 0:
        print("!!! [PositiveAssignment Debug] bbox too small")
        print("  scale:", scale)
        print("  bbox:", bbox)
        print("  scaled_bbox:", scaled_bbox)
        print("  rasterized_scaled_bbox:", rasterized_scaled_bbox)
    assert len(positive_sample_indices) > 0, (
        f'bbox is too small.\n'
        f'scale: {scale}\n'
        f'bbox: {bbox}\n'
        f'rasterized_scaled_bbox: {rasterized_scaled_bbox}\n'
        f'scaled_bbox: {scaled_bbox}'
    )
...
class BoxWithScoreMapLabelGenerator:
    ...
    def __call__(self, training_pair, context: dict, data: dict, _):
        if context['is_positive']:
            ...
            # print(f"[LabelGen DBG] x_cropped_bbox (px coords) = {x_cropped_bbox}")

            positive_sample_indices = positive_sample_assignment(x_cropped_bbox, self.response_map_size,
                                                                 self.search_region_size)

            # print(f"[LabelGen DBG] positive_sample_indices count = {len(positive_sample_indices)}")
            ...
            # print(f"[LabelGen DBG] normalized_bbox ([0,1] coords) = {normalized_bbox}")
