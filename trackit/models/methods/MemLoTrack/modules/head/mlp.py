...
class MlpAnchorFreeHead(nn.Module):
    ...
    def forward(self, x):
        ...
        # print(f"[Head DBG] input x: shape={tuple(x.shape)}, NaN? {x.isnan().any().item()}")
        W, H = self.map_size
        bbox_offset = self.bbox_offset

        # print(f"[Head Debug] INPUT x shape={x.shape}, NaN? {x.isnan().any().item()}")
        # print(f"[Head Debug] BEFORE cls_mlp: x NaN? {x.isnan().any().item()}")
        score_map = self.cls_mlp(x).to(torch.float32)
        box_map   = self.reg_mlp(x).to(torch.float32)
        # print(f"[Head DBG] after MLP: score_map NaN? {score_map.isnan().any().item()}")

        # print(f"[Head DBG] after MLP: box_map NaN? {box_map.isnan().any().item()}, "
        #       f"min={box_map.min():.4f}, max={box_map.max():.4f}")

        # 기존 로직 계속
        B, L, C = score_map.shape                # (B, H*W, 1)
        score_map = score_map.view(B, H, W)      # (B, H, W)
