...
class PlainWrapper(nn.Module):
    ...
    def forward(self, data):
        with torch.inference_mode(), self.amp_autocast_fn():
            # 1) Print the type or keys of incoming data
            if isinstance(data, dict):
                # print("[PlainWrapper Debug] before auto_unpack => data keys:", list(data.keys()))
                # 2) Force return_x_feat=True
                data["return_x_feat"] = True
            else:
                # print("[PlainWrapper Debug] data is not a dict => type:", type(data))
                pass
            
            # 3) Actually call the model
            out = auto_unpack_and_call(data, self.model)
            
            # 4) Print the result (whether dict or something else)
            # if isinstance(out, dict):
            #     print("[PlainWrapper Debug] model output => keys:", list(out.keys()))
            # else:
            #     print("[PlainWrapper Debug] model output => not a dict. type:", type(out))
...
class PlainInferenceEngine(InferenceEngine):
    ...
    def __call__(self, 
                 ...
                 device: torch.device, _):
        model = model_manager.create(device, 
                                     ModelImplSuggestions(optimize_for_inference=False)).model
        model.eval()
        ...
        if self._torch_compile_option.enabled:
            if self._torch_compile_option.options is None:
                ...
                # print('[PlainInferenceEngine] torch.compile is enabled (no extra options).')
            else:
                optimized_model = torch.compile(optimized_model, **self._torch_compile_option.options)
                # print(f'[PlainInferenceEngine] torch.compile is enabled with options: {self._torch_compile_option.options}.')
        ...
        if self._enable_data_parallel and should_use_data_parallel(device):
            ...
            # print('[PlainInferenceEngine] DataParallel is enabled.')

        amp_autocast_fn = get_torch_amp_autocast_fn(
            device.type, 
            self._auto_mixed_precision_option.enabled, 
            self._auto_mixed_precision_option.dtype)
        
        # print("[PlainInferenceEngine] returning PlainWrapper(optimized_model, amp_autocast_fn)")
